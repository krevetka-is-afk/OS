...
void readProcess(int *fdReader, char *buffer, const char *input) {
    close(fdReader[0]);  // Закрываем конец чтения
    ...
    close(fdReader[1]);  // Закрываем конец записи
}

void HandleProcess(int *fdWriter, int *fdReader, char *buffer) {
    close(fdReader[1]);  // Закрываем неиспользуемый конец записи
    ...
    close(fdReader[0]);  // Закрываем конец чтения после использования

    ssize_t count = findKeyWord(buffer, buffer);

    close(fdWriter[0]);  // Закрываем конец чтения (он не используется в этом процессе)
...
void WriteProcess(int *fdWriter, char *buffer, const char *output_name) {
    ...
    close(fdWriter[0]);  // Закрываем после чтения
...
int main(int argc, char **argv) {
    ...
    // Родительский процесс (читаем файл, записываем результат)
